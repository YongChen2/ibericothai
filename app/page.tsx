  ...
  return (
    <main className="min-h-screen bg-[#f8f5ef] text-[#1d1d1d] selection:bg-[#d8b36a] selection:text-black">
      ...
      <section id="menu" className="relative z-10 mx-auto max-w-7xl px-6 py-24 lg:px-10">
        ...
        <div className="mt-10">
          <a href="/menu.pdf" target="_blank" rel="noreferrer" className="inline-flex rounded-full bg-[#d8b36a] px-7 py-4 text-sm font-medium text-black transition hover:scale-[1.03] hover:shadow-lg">{current.menu.button}</a>
        </div>
      </section>

      <section id="locations" className="relative z-10 mx-auto max-w-7xl px-6 py-24 lg:px-10">
        <div className="mb-14 max-w-2xl">
          <p className="mb-4 text-sm uppercase tracking-[0.35em] text-[#b8862f]">{current.locations.badge}</p>
          <h2 className="text-4xl font-semibold tracking-tight text-[#1c1a17] sm:text-5xl">{current.locations.title}</h2>
          <p className="mt-5 text-lg leading-8 text-[#5d5549]">{current.locations.text}</p>
        </div>

        <div className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-[2rem] border border-[#e2d6bf] bg-white/80 p-8 shadow-sm">
            <p className="text-sm uppercase tracking-[0.25em] text-[#b8862f]">Location 01</p>
            <h3 className="mt-4 text-3xl font-semibold text-[#1c1a17]">{current.locations.first.name}</h3>
            <p className="mt-4 text-[#5d5549]">{current.locations.first.desc}</p>
            <div className="mt-8 space-y-4 text-[#554d42]">
              <p>{current.locations.first.address}</p>
              <p>{current.locations.first.hours}</p>
              <a href={`tel:${current.locations.first.phone}`} className="block text-[#9d7230] hover:text-[#b8862f]">{current.locations.first.phone}</a>
            </div>
            <a href="https://maps.google.com" target="_blank" rel="noreferrer" className="mt-8 inline-flex rounded-full border border-[#d4c3a1] bg-[#fffaf1] px-6 py-3 text-sm font-medium text-[#2b2722] transition hover:border-[#b8862f] hover:text-[#b8862f]">{current.locations.cta}</a>
          </div>

          <div className="rounded-[2rem] border border-[#e2d6bf] bg-white/80 p-8 shadow-sm">
            <p className="text-sm uppercase tracking-[0.25em] text-[#b8862f]">Location 02</p>
            <h3 className="mt-4 text-3xl font-semibold text-[#1c1a17]">{current.locations.second.name}</h3>
            <p className="mt-4 text-[#5d5549]">{current.locations.second.desc}</p>
            <div className="mt-8 space-y-4 text-[#554d42]">
              <p>{current.locations.second.address}</p>
              <p>{current.locations.second.hours}</p>
              <a href={`tel:${current.locations.second.phone}`} className="block text-[#9d7230] hover:text-[#b8862f]">{current.locations.second.phone}</a>
            </div>
            <a href="https://maps.google.com" target="_blank" rel="noreferrer" className="mt-8 inline-flex rounded-full border border-[#d4c3a1] bg-[#fffaf1] px-6 py-3 text-sm font-medium text-[#2b2722] transition hover:border-[#b8862f] hover:text-[#b8862f]">{current.locations.cta}</a>
          </div>
        </div>
      ...
      <section id="gallery" className="relative z-10 mx-auto max-w-7xl px-6 py-24 lg:px-10">
        <div className="mb-14 max-w-2xl">
          <p className="mb-4 text-sm uppercase tracking-[0.35em] text-[#b8862f]">{current.gallery.badge}</p>
          <h2 className="text-4xl font-semibold tracking-tight text-[#1c1a17] sm:text-5xl">{current.gallery.title}</h2>
          <p className="mt-5 text-lg leading-8 text-[#5d5549]">{current.gallery.text}</p>
        </div>

        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {galleryImages.map((image, index) => (
            <div key={index} className="overflow-hidden rounded-[1.8rem] border border-[#e2d6bf] bg-white/80 shadow-sm">
              <img src={image} alt={`Gallery ${index + 1}`} className="h-80 w-full object-cover transition duration-500 hover:scale-105" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="about" className="relative z-10 mx-auto max-w-7xl px-6 py-24 lg:px-10">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[2rem] border border-[#e2d6bf] bg-white/80 p-8 shadow-sm sm:p-10">
            <p className="mb-4 text-sm uppercase tracking-[0.35em] text-[#b8862f]">{current.about.badge}</p>
            <h2 className="text-4xl font-semibold tracking-tight text-[#1c1a17] sm:text-5xl">{current.about.title}</h2>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#5d5549]">{current.about.text1}</p>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-[#5d5549]">{current.about.text2}</p>
          </div>

          <div className="rounded-[2rem] border border-[#e2d6bf] bg-white/80 p-8 shadow-sm">
            <p className="mb-6 text-sm uppercase tracking-[0.25em] text-[#b8862f]">Highlights</p>
            <div className="grid gap-4">
              {current.about.points.map((point) => (
                <div key={point} className="rounded-2xl border border-[#eadfca] bg-[#fffaf1] px-5 py-4 text-[#554d42]">
                  {point}
                </div>
              ))}
            </div>
      ...
      <section id="contact" className="relative z-10 mx-auto max-w-7xl px-6 py-24 lg:px-10">
        <div className="rounded-[2rem] border border-[#e2d6bf] bg-white/80 p-8 shadow-sm sm:p-10">
          <p className="mb-4 text-sm uppercase tracking-[0.35em] text-[#b8862f]">{current.contact.badge}</p>
          <div className="grid gap-10 lg:grid-cols-[1fr_0.9fr]">
            <div>
              <h2 className="text-4xl font-semibold tracking-tight text-[#1c1a17] sm:text-5xl">{current.contact.title}</h2>
              <p className="mt-6 max-w-xl text-lg leading-8 text-[#5d5549]">{current.contact.text}</p>
            </div>

            <div className="grid gap-4">
              <a href="/menu.pdf" target="_blank" rel="noreferrer" className="rounded-full bg-[#d8b36a] px-7 py-4 text-center text-sm font-medium text-black transition hover:scale-[1.02] hover:shadow-lg">{current.contact.menuBtn}</a>
              <a href="tel:+34931895352" className="rounded-full border border-[#d4c3a1] bg-[#fffaf1] px-7 py-4 text-center text-sm font-medium text-[#2b2722] transition hover:border-[#b8862f] hover:text-[#b8862f]">{current.contact.phoneBtn}</a>
              <a href="https://maps.google.com" target="_blank" rel="noreferrer" className="rounded-full border border-[#d4c3a1] bg-[#fffaf1] px-7 py-4 text-center text-sm font-medium text-[#2b2722] transition hover:border-[#b8862f] hover:text-[#b8862f]">{current.contact.mapBtn}</a>
            </div>
          </div>
        </div>
      </section>

      <footer className="relative z-10 border-t border-[#dfd1b2]">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-6 py-8 text-sm text-[#6d6559] md:flex-row lg:px-10">
          <div className="flex items-center gap-3">
            <img src={logoSrc} alt="Iberico Thai logo" className="h-8 w-8 rounded-full border border-[#d8c8a5] bg-white object-cover p-1" />
            <span className="tracking-[0.3em] text-[#5f564a]">Iberico Thai</span>
          </div>
          <p>{current.footer}</p>
        </div>
      </footer>
